...
pub const App = struct {
    ...
    pub fn Activate(self: *App, allocator: std.mem.Allocator) !void {
        ...
        if (rootWidget) |root| {
            var container_template: AppWidget = .{ .label = "LeftPanel", .widgetType = gui.widgets.WidgetType(App){ .Container = .{ .allocator = allocator } }, .size = .{ .x = 480, .y = 780 }, .color = .{ .r = 50, .g = 100, .b = 100, .a = 255 }, .transform = .{ .position = .{ .x = 10, .y = 10 } } };

            self.left_panel = root.*.addChildWidget(container_template);

            container_template = .{ .label = "RightPanel", .widgetType = gui.widgets.WidgetType(App){ .Container = .{ .allocator = allocator } }, .size = .{ .x = 480, .y = 780 }, .color = .{ .r = 100, .g = 50, .b = 100, .a = 255 }, .transform = .{ .position = .{ .x = 510, .y = 10 } } };

            self.right_panel = root.*.addChildWidget(container_template);

            container_template = .{ .label = "SmallPanel", .widgetType = gui.widgets.WidgetType(App){ .Container = .{ .allocator = allocator } }, .size = .{ .x = 460, .y = 340 }, .color = .{ .r = 150, .g = 150, .b = 100, .a = 255 }, .transform = .{ .position = .{ .x = 10, .y = 350 } } };
            ...
            const checkbox_template: AppWidget = .{
                .label = "check",
                .widgetType = gui.widgets.WidgetType(App){ .CheckBox = gui.widgets.CheckBox(App){ .onCheckStateChanged = OnCheckToggle } },
                .size = .{ .x = 50, .y = 50 },
                .color = .{ .r = 200, .g = 200, .b = 200, .a = 255 },
